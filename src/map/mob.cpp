 ...
 */
 static int mob_readdb_hunting(void)
 {
 	...
 	{
 		ShowError("mob_readdb_hunting: Can't read %s\n", line);
 		return -1;
 	}
 	...
 	fclose(fp);
 	ShowStatus("Done reading '" CL_WHITE "%d" CL_RESET "' entries in '" CL_WHITE "mob_hunting.txt" CL_RESET "'.\n",ln);
 	return 0;
 }
...
int mob_get_random_id_lv(int lv, int range)
{
	...
	int lv_min = lv - range;
	int lv_max = lv + range;
	int Mob_id[MAX_MOB_DB];

	// Count Mobs
	for( i = MIN_MOB_DB; i < MAX_MOB_DB; i++ )
	{
		mob = mob_db(i);
		if( mob == NULL || !mob->hunting || mob->lv < lv_min || mob->lv > lv_max ||
		(status_has_mode(&mob->status,MD_STATUS_IMMUNE) ) ||
		(!mob_has_spawn(i)) ||
