...
// (^~_~^) Auras Start
	...
	sd->aura_data = pc_readglobalreg(sd, add_str("AURA_DATA"));
	sd->state.show_auras = pc_readglobalreg(sd, add_str("SHOW_AURAS_MODE"));
...
// (^~_~^) LGP End
	...
	// Hunting Missions [DanielArt]
	sd->hunting_time = static_cast<short>(pc_readglobalreg(sd, add_str(MISSION_TICK)));
...
{
	int item_id = 0;
	int item_card2 = 0; //Aglao - Variable para asignar valor de card2 en item consumido
	int item_card3 = 0; //Aglao - Variable para asignar valor de card3 en item consumido
	int item_quantity = 0; //Aglao - Variable para asignar valor de quantity en item consumido
	
	nullpo_retr(1, sd);
	
	item_card2 = sd->inventory.u.items_inventory[n].card[2]; //Aglao - Asignacion valor de card2 en item consumido
	item_card3 = sd->inventory.u.items_inventory[n].card[3]; //Aglao - Asignacion valor de card3 en item consumido
	item_quantity = amount; //Aglao - Asignacion valor de quantity en item consumido
	...
	if(log_type == LOG_TYPE_CONSUME) {
		...
		pc_setparam(sd, SP_ITEMUSEDCARD2, item_card2); //Aglao - Se agrega asignacion de valor Card2 al item consumido.
		pc_setparam(sd, SP_ITEMUSEDCARD3, item_card3); //Aglao - Se agrega asignacion de valor Card3 al item consumido.
		pc_setparam(sd, SP_ITEMUSEDQUANTITY, item_quantity); //Aglao - Se agrega asignacion de valor Quantity al item consumido.
...
int pc_readparam(struct map_session_data* sd,int type)
{
	...
	switch(type) {
		...
		case SP_ITEMUSEDCARD2:   val = sd->itemusedcard2; break; //Aglao - Case de validacion de propiedad card2 de item consumido
		case SP_ITEMUSEDCARD3:   val = sd->itemusedcard3; break; //Aglao - Case de validacion de propiedad card3 de item consumido
		case SP_ITEMUSEDQUANTITY:   val = sd->itemusedquantity; break; //Aglao - Case de validacion de propiedad quantity de item consumido
...
bool pc_setparam(struct map_session_data *sd,int type,int val)
{
    ...
    case SP_ITEMUSEDID:
		sd->itemusedid = val;
		return true;
	case SP_ITEMUSEDCARD2: //Aglao - Asignacion de valor Card2 al item consumido
		sd->itemusedcard2 = val;
		return true;
	case SP_ITEMUSEDCARD3: //Aglao - Asignacion de valor Card3 al item consumido
		sd->itemusedcard3 = val;
		return true;
	case SP_ITEMUSEDQUANTITY: //Aglao - Asignacion de valor Quantity al item consumido
		sd->itemusedquantity = val;
		return true;
	case SP_CHARMOVE:
