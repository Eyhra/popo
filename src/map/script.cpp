...
BUILDIN_FUNC(mission_sethunting)
{
	...
	if( index < 1 || index > MAX_HUNTING_MOB ) {
		ShowError("buildin_mission_sethunting: Wrong array hunting (%d).\n",index);
		return SCRIPT_CMD_FAILURE; // Invalid Index
	}
...
// (^~_~^) Gepard Shield End
	...
	BUILDIN_DEF(checkisdead, "i"), // [DanielArt]
